...
func Input(name string) []remove_duplicates.Data {
	...
	defer csvFile.Close()
	csv_reader := csv.NewReader(bufio.NewReader(csvFile))
	csv_reader.Comma = ';'
...
func Algorithm_Starter(data []remove_duplicates.Data) {
	...
	var (
		GlobalValueDataHolder [][]string
		GlobalPathDataHolder []RouteInfo
		GlobalFromToDataHolder []string
		counter1 = 0
	)
...
func get_info_stations(edges []string, NewData InfoArray, data []remove_duplicates.Data) {
	...
	fmt.Println(edges)
	cur_station_reader := bufio.NewReader(os.Stdin)
	fmt.Println("Enter your current station: ")
	cur_station_holder, _ := cur_station_reader.ReadString('\n')
	cur_station_holder = strings.Replace(cur_station_holder, "\n", "", -1)

	destination_reader := bufio.NewReader(os.Stdin)
